...
function renderHome() {
  ...
  // 매트릭스
  if (matrixGrid) {
    const collab = productsData.collab.slice(0, 6);
    matrixGrid.innerHTML = `
      ...
      <a class="matrix-main" href="sub1.html">
        <div class="matrix-banner-content">
          <div class="matrix-banner-badge">Collaboration</div>
          <div class="matrix-banner-title">러쉬 X 헬로키티와 친구들</div>
          <div class="matrix-banner-desc">선 론칭! 지금 바로 확인하세요.</div>
        </div>
      </a>
    `;
  }
...
function initLiveNotification() {
  ...
  const products = ['슬리피 샤워 젤', '바나나아 샤워 크림', '더티 바디스프레이', '대디-오 샴푸'];
  const names = ['김**', '이**', '박**', '최**'];
  let i = 0;
